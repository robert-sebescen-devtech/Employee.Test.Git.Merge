...
namespace Employee.Test.Git.Merge.Services
{
    public class TestService : ITestService
    {
        ...
        public List<string> PeopleNames()
        {
            ...
            List<string> noShortFirstNames = (peopleNames.Where(name => name.Split(' ')[0].Length > 3).ToList()).Select(name => name.ToUpper()).ToList();
            
            //noShortFirstNames.forEach(name => name.ToUpper());
